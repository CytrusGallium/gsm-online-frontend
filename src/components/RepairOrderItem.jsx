...
const RepairOrderItem = (props) => {
  ...
  const [itemState, setItemState] = useState({ deviceType: "", ref: "", imei: "", problem: "", estPrice: "0", price: "0", state: "Encours de réparation..." });
  ...
  const updateProblemList = (ParamProblems) => {
    console.log("Problems = " + JSON.stringify(ParamProblems.problems));
  }
  ...
  return (
    <div className='w-64 flex flex-col border-2 border-gray-300 mt-2 py-2 rounded-lg'>
      ...
      <br />
      <select name="state" className={inputStyle} value={itemState.state} onChange={handleChange}>
      ...
      <br />
      {props.id != 1 && <button type="button" className={buttonStyle} onClick={handleRemove}>Enlever Cette Appareil</button>}
    </div>
  )
}
