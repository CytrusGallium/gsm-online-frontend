...
import ComputerSpecsManager from './pages/ComputerSpecsManager';

function App() {

  const logo = require(`./${AppData.LOGO}`); // relative path to image 
      ...
      <div className='mt-16' id='app-module-container'>
        <BrowserRouter>
          ...
          <nav className='px-2 sm:px-4 py-2.5 bg-gray-900 fixed w-full z-20 top-0 left-0 border-b border-gray-600 h-16'>
            <h1 className="text-gray-100 font-bold text-3xl text-left ml-12">
              <Link to='/Dashboard'><span className='opacity-50 hover:opacity-90 duration-300'>{AppData.COMPANY_LABEL}</span></Link>
            </h1>
            <img style={{top:AppData.LOGO_TOP_DISTANCE}} src={logo} className='absolute left-4 w-8 w-8'></img>
          ...
          <Routes>
            ...
            {AppData.COMPUTER_SPECS_FLAG && <Route path='/computer-specs-manager' element={IsLoggedIn() ? <ComputerSpecsManager /> : <PleaseLogin />} />}
